...
const NewEnquiryPage = () => {
  ...
  const { data: enquiryData, isLoading: isLoadingEnquiryData } =
    useGetEnquiryDetail(enquiry_id ?? "");
  ...
  console.log("enquiryData data", enquiryData?.delivery?.delivery_date);
  ...
  React.useEffect(() => {
    if (!isLoadingEnquiryData && !!enquiryData) {
      const resetData: Partial<NewEnquiryFormValues> = {
        ...
        delivery: {
          zone:
            enquiryData.delivery?.zone || ("LM" as "LM" | "LC" | "LI" | "ND"),
          method: enquiryData.delivery?.method as
            | "Dispatch"
            | "Pickup"
            | undefined,
          ...
          recipient_alternative_phone:
            enquiryData.delivery?.recipient_alternative_phone || "",
          residence_type: enquiryData.delivery?.residence_type || "",
          note: enquiryData.delivery?.note || "",
          fee: enquiryData.delivery?.fee
            ? Number(enquiryData.delivery.fee)
            : undefined,
          ...
          delivery_time: enquiryData.delivery?.delivery_time
            ? format(
                new Date(`1970-01-01T${enquiryData.delivery.delivery_time}`),
                "HH:mm",
              )
            : "15:00",
        },
            ...
            inventories: item.inventories?.map((inventory) => ({
              ...
              variations:
                inventory.variations?.map((variation) => ({
                  stock_variation_id: !!inventory.stock_inventory
  ...
  const onSubmit = async (data: NewEnquiryFormValues) => {
    ...
    mutate(
      { id: enquiry_id ?? "", data: dataToSubmit },
      {
        onSuccess(data) {
                ...
                <AccordionContent className="flex flex-col pt-3 pb-14 gap-y-8">
                  {enquiryData?.metadata?.line_items?.length ? (
                    <div className="flex flex-col gap-6">
                      {enquiryData.metadata.line_items.map(
